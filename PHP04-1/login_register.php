...
$pd = '123456';
$db = 'xah181006';
$con = new mysqli($sn,$un,$pd,$db); //数据库连接的对象
//判断数据库是否连接成功
...
//解析前端HTTP请求
//$_POST中含有这次请求发送过来的所有数据
//$_POST数据类型是关联数组

//跟据前端传的type值，确定当前的操作类型
$type = $_POST['type'];
switch ($type) {
    case 'login':
        login();
...
function login() {
    ...
    $uname = $_POST['uname'];
    $pswd = $_POST['pswd'];
    //执行sql语句作对比，查询该用户对应的密码，查到的条目数为0，代表用户不存在，查到的密码和用户名输入的密码不相等，代表密码错误，第三种就是登陆成功。
...
function register() {
    ...
    //$testRes是对象，这个对象的num_rows键代表查询的数据条目，如果这个用户不存在，那么num_rows为0，可以注册
